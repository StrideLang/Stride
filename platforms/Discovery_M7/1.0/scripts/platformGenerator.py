...
class Generator:
    ...
    def generate_code(self):
        ...
        if os.path.exists(self.out_dir + "/project"):
...
#            flags =  '--upload --board arduino:avr:uno --port /dev/ttyACM0 --pref sketchbook.path=' + self.platform_dir + '/sketchbook'            
            ...
            args = [self.platform_dir + '/stlink/st-flash', 'write', 'blinky.bin', '0x8000000'] #+ flags.split() + [self.out_file]
            outtext = ck_out(args, cwd=self.out_dir + "/project")
            self.log(outtext)
